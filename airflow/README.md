...
|------|----------|--------------|
| `dbt_run` | `DatabricksRunNowOperator` | Execute dbt models (staging, silver, gold) |
...
- **`DatabricksPartitionSensor`** -- Imported but reserved for future use (e.g., waiting for upstream data partitions before triggering the pipeline).
...
2. **Single Databricks job** -- All tasks reference the same `job_id` with different parameters. This maps to the multi-task Databricks job defined in `databricks/job_config.json`. Airflow controls the execution order; Databricks handles compute.
...
```bash
# Validate DAG syntax (requires Airflow installed)
airflow dags test vusion_dbt_pipeline 2024-01-01
```

The DAG file is linted by ruff as part of the pre-commit pipeline.
